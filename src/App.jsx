...
const ADDRESSES = [
  { id:1, name:"ობიექტი I – რუსთაველის გამზ. 15, ვაკე",       short:"ობიექტი I · ვაკე",       color:"#D32F2F", light:"#FFF5F5", darkLight:"#1A0808" },
  { id:2, name:"ობიექტი II – ჭავჭავაძის გამზ. 45, საბურთალო", short:"ობიექტი II · საბურთალო", color:"#1D4ED8", light:"#EFF6FF", darkLight:"#080D1A" },
  { id:3, name:"ობიექტი III – კახეთის გზ. 77, ისანი",         short:"ობიექტი III · ისანი",     color:"#16A34A", light:"#F0FDF4", darkLight:"#081208" },
];
...
const INIT_CATS = [
  ...
  { id:6, name:"უსაფრთხოების აღჭურვილობა", icon:"🦺", color:"#16A34A" },
  { id:7, name:"ტექნიკა და გენერატორები",   icon:"⚙️", color:"#374151" },
...
const ITEMS0 = [
  ...
  { id:uid(), name:"სამუშაო ხელთათმანები", catId:6, unit:"წყვ", qty:80,   minQ:20,  price:1.80, addrId:3, notes:"" },
...
const PURCH0 = [
  { id:uid(), date:"2026-01-15", item:"ცემენტი M400",       catId:1, addrId:1, qty:3000, unit:"კგ", up:0.45, sup:"სამშენებლო სუპერმარკეტი", status:"დასრულებული", notes:"" },
  { id:uid(), date:"2026-02-10", item:"ელ. კაბელი 2.5mm²", catId:2, addrId:3, qty:500,  unit:"მ",  up:1.20, sup:"ელექტრო ბაზარი",          status:"დასრულებული", notes:"" },
  { id:uid(), date:"2026-03-02", item:"მილი PVC D50",       catId:3, addrId:1, qty:80,   unit:"მ",  up:4.80, sup:"სანტექნიკა პლუს",         status:"მოლოდინი",   notes:"10 მარტი" },
];

const SPENDS0 = [
  { id:uid(), date:"2026-01-20", desc:"ბეტონის სამუშაო – საფუძველი",   catId:1, addrId:1, amount:4500, status:"დამტკიცებული", notes:"" },
  { id:uid(), date:"2026-02-08", desc:"ელექტრო სადინარი – სარდაფი",    catId:2, addrId:3, amount:1200, status:"დამტკიცებული", notes:"" },
  { id:uid(), date:"2026-03-03", desc:"კარ-ფანჯრების კომპლექტი",       catId:5, addrId:2, amount:7800, status:"მოლოდინი",    notes:"" },
];
...
function KCard({ label, value, icon, sub, G }) {
  return (
    <div style={{ background:G.card, borderRadius:12, padding:"18px 20px", boxShadow:G.shadow, position:"relative", overflow:"hidden", border:`1px solid ${G.border}` }}>
      ...
      <div style={{ fontSize:28, fontWeight:800, color:G.text, lineHeight:1, letterSpacing:-.5 }}>{value}</div>
...
const NAV = [
  { id:"dashboard",  label:"მთავარი გვერდი", icon:"⬛" },
...
export default function App() {
  ...
  const notifs = [
    ...
    ...spends.filter(s => s.status === "მოლოდინი" && cfg.approval).map(s => ({ id:"s"+s.id, msg:`💸 "${s.desc}" — დამტკიცება საჭიროა`, c:G.amber })),
  ];
  ...
  function PageDashboard() {
    ...
    const recent = [
      ...purch.slice(0,5).map(p => ({ ...p, _t:"შესყიდვა", _n:p.item, _v:p.qty * p.up })),
      ...spends.slice(0,5).map(s => ({ ...s, _t:"ხარჯი", _n:s.desc, _v:s.amount })),
    ].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 8);

    return <>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        <h2 style={{ fontSize:22, fontWeight:800, color:G.text, margin:0 }}>მთავარი გვერდი</h2>
        <div style={{ display:"flex", gap:8 }}>
          <Btn size="sm" variant="secondary" G={G} onClick={() => setModal("purch")}>🛒 შესყიდვა</Btn>
          <Btn size="sm" G={G} onClick={() => setModal("spend")}>💸 ხარჯი</Btn>
        </div>
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(175px,1fr))", gap:12, marginBottom:22 }}>
        <KCard G={G} label="ინვენტარის ღირებულება" value={`${cur}${invVal.toLocaleString("ka-GE",{maximumFractionDigits:0})}`} icon="💰" />
        <KCard G={G} label={`${MONTHS_F[cm]} შესყიდვები`} value={`${cur}${thisPur.reduce((s,p)=>s+p.qty*p.up,0).toFixed(0)}`} icon="🛒" />
        <KCard G={G} label={`${MONTHS_F[cm]} ხარჯები`} value={`${cur}${thisSp.reduce((s,sp)=>s+sp.amount,0).toFixed(0)}`} icon="💸" />
        <KCard G={G} label="მცირე მარაგი" value={lowItems.length} icon="⚠️" sub={lowItems.length>0?"გაფრთხილება საჭიროა":"ყველა ნორმალურია"} />
        <KCard G={G} label="პოზიციები" value={items.length} icon="📦" />
        <KCard G={G} label="ობიექტები" value={ADDRESSES.length} icon="🏗" />
      </div>

      <div style={{ fontSize:12, fontWeight:700, color:G.textMuted, textTransform:"uppercase", letterSpacing:.8, marginBottom:12 }}>ობიექტების მდგომარეობა</div>
        ...
        {addrSt.map(a => (
          <div key={a.id} style={{ background:G.card, borderRadius:12, padding:"18px 20px", boxShadow:G.shadow, borderTop:`3px solid ${a.color}`, border:`1px solid ${G.border}` }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
              {[["შესყიდვები",`${cur}${a.pT.toFixed(0)}`],["ხარჯები",`${cur}${a.sT.toFixed(0)}`],["პოზიციები",a.cnt]].map(([l,v]) => (
                <div key={l} style={{ background:G.cardAlt, borderRadius:8, padding:"8px 10px" }}>
                  ...
                  <div style={{ fontSize:16, fontWeight:800, color:G.text, marginTop:3 }}>{v}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        <div>
          ...
          {lowItems.length === 0
            ? <div style={{ background:G.card, borderRadius:12, padding:24, textAlign:"center", color:G.green, fontSize:14, fontWeight:600, boxShadow:G.shadow, border:`1px solid ${G.border}` }}>✅ ყველა ნორმალურია</div>
            : <div style={{ background:G.card, borderRadius:12, boxShadow:G.shadow, overflow:"hidden", border:`1px solid ${G.border}` }}>
                {lowItems.map((i, idx) => (
        ...
        <div>
          <div style={{ fontSize:12, fontWeight:700, color:G.textMuted, textTransform:"uppercase", letterSpacing:.8, marginBottom:12 }}>🕐 ბოლო ოპერაციები</div>
          <div style={{ background:G.card, borderRadius:12, boxShadow:G.shadow, overflow:"hidden", border:`1px solid ${G.border}` }}>
            {recent.map((r, i) => (
              <div key={r.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 14px", borderBottom:i<recent.length-1?`1px solid ${G.border}`:"none" }}>
                <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                  <span style={{ width:28, height:28, borderRadius:8, background:r._t==="შესყიდვა"?"#DBEAFE":"#FEF9C3", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, flexShrink:0 }}>
                    {r._t === "შესყიდვა" ? "🛒" : "💸"}
                  </span>
                  <div>
                ...
                <div style={{ textAlign:"right" }}>
                  <div style={{ fontSize:14, fontWeight:800, color:r._t==="შესყიდვა"?G.blue:G.accent }}>{cur}{r._v?.toFixed(0)}</div>
  ...
  function PageInventory() {
    ...
    return <>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
        ...
        <Btn G={G} onClick={() => setModal("item")}>+ ახალი პოზიცია</Btn>
      </div>
      <Filters G={G} addr={fA} setAddr={setFA} month={fM} setMonth={setFM} cat={fC} setCat={setFC} status={undefined} setStatus={setFSt} cats={cats}
        extra={<SearchBox G={G} value={sI} onChange={setSI} placeholder="ძებნა სახელით..." />} />
      <DataTable G={G}
        cols={[
          ...
          { k:"cat",   label:"კატეგორია",   fn:r => { const c = cats.find(x => x.id === r.catId); return <span style={{ fontSize:13, color:G.textSecondary }}>{c?.icon} {c?.name}</span>; }},
          { k:"addr",  label:"ობიექტი",     fn:r => <APill addrId={r.addrId} dark={dark} /> },
          { k:"qty",   label:"რაოდენობა",   fn:r => <span style={{ fontWeight:700, color:r.qty<=r.minQ?G.accent:G.text, fontSize:14 }}>{r.qty} {r.unit}</span> },
          { k:"min",   label:"მინიმუმი",    fn:r => <span style={{ fontSize:13, color:G.textMuted }}>{r.minQ} {r.unit}</span> },
          ...
          { k:"st",    label:"სტატუსი",     fn:r => r.qty<=r.minQ
              ? <Badge status="მოლოდინი" dark={dark}/>
              : <span style={{ background:dark?"#0A1F10":"#DCFCE7", color:dark?"#4ADE80":"#166534", borderRadius:20, padding:"3px 10px", fontSize:12, fontWeight:700 }}>ნორმა</span> },
        ...
        rows={fItems}
        footer={[
          <span key="c" style={{ fontSize:13, color:G.textSecondary }}>სულ: <strong>{fItems.length}</strong> პოზიცია</span>,
          <span key="v" style={{ fontSize:13, color:G.textSecondary }}>ჯამური ღირებულება: <strong style={{ color:G.green }}>{cur}{tv.toFixed(2)}</strong></span>,
          <span key="l" style={{ fontSize:13, color:G.accent }}>მცირე მარაგი: <strong>{fItems.filter(i => i.qty <= i.minQ).length}</strong></span>,
        ]}
      />
    </>;
  }
  ...
  function PageCategories() {
    return <>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
        ...
        <Btn G={G} onClick={() => setModal("cat")}>+ ახალი კატეგორია</Btn>
      </div>
  ...
  function PagePurchases() {
    ...
    return <>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
        ...
        <Btn G={G} onClick={() => setModal("purch")}>🛒 + შესყიდვა</Btn>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:12, marginBottom:16 }}>
        <KCard G={G} label="ჩანაწერები" value={fPurch.length} icon="📋" />
        <KCard G={G} label="სულ რაოდენობა" value={tq.toFixed(0)} icon="📦" />
        <KCard G={G} label="თანხა" value={`${cur}${tot.toFixed(0)}`} icon="💰" />
        <KCard G={G} label="დღგ" value={`${cur}${(tot*+cfg.tax/100).toFixed(0)}`} icon="🧾" sub={`${cfg.tax}%`} />
      </div>
      ...
      <DataTable G={G}
        cols={[
          { k:"date", label:"თარიღი",      fn:r => <span style={{ fontSize:13, color:G.textMuted }}>{r.date}</span> },
          { k:"item", label:"საქონელი",    fn:r => <span style={{ fontSize:14, fontWeight:700 }}>{r.item}</span> },
          { k:"cat",  label:"კატეგორია",   fn:r => { const c = cats.find(x => x.id === r.catId); return <span style={{ fontSize:13 }}>{c?.icon} {c?.name}</span>; }},
          { k:"addr", label:"ობიექტი",     fn:r => <APill addrId={r.addrId} dark={dark} /> },
          { k:"qty",  label:"რაოდენობა",   fn:r => <span style={{ fontWeight:700, color:G.blue, fontSize:14 }}>{r.qty} {r.unit}</span> },
          { k:"up",   label:"ფასი/ერთ.",   fn:r => <span style={{ fontSize:13 }}>{cur}{r.up}</span> },
          { k:"tot",  label:"ჯამი",        fn:r => <span style={{ fontWeight:700, color:G.green, fontSize:14 }}>{cur}{(r.qty*r.up).toFixed(2)}</span> },
          { k:"sup",  label:"მომწოდებელი", fn:r => <span style={{ fontSize:13, color:G.textSecondary }}>{r.sup}</span> },
          { k:"st",   label:"სტატუსი",     fn:r => (
        ...
        rows={fPurch}
        footer={[
          <span key="t" style={{ fontSize:13, color:G.textSecondary }}>ჯამი: <strong style={{ color:G.green }}>{cur}{tot.toFixed(2)}</strong></span>,
          <span key="q" style={{ fontSize:13, color:G.textSecondary }}>სულ რაოდენობა: <strong>{tq.toFixed(1)}</strong></span>,
        ]}
      />
    </>;
  }
  ...
  function PageSpends() {
    ...
    return <>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
        ...
        <Btn G={G} onClick={() => setModal("spend")}>💸 + ხარჯი</Btn>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:12, marginBottom:16 }}>
        <KCard G={G} label="ჩანაწერები" value={fSpend.length} icon="📋" />
        <KCard G={G} label="სულ" value={`${cur}${tot.toFixed(0)}`} icon="💸" />
        <KCard G={G} label="დამტკიცებული" value={`${cur}${appr.toFixed(0)}`} icon="✅" />
        <KCard G={G} label="მოლოდინი" value={`${cur}${pend.toFixed(0)}`} icon="⏳" />
      </div>
      ...
      <DataTable G={G}
        cols={[
          { k:"date",   label:"თარიღი",   fn:r => <span style={{ fontSize:13, color:G.textMuted }}>{r.date}</span> },
          { k:"desc",   label:"აღწერა",   fn:r => <span style={{ fontSize:14, fontWeight:700, color:G.text }}>{r.desc}</span> },
          { k:"cat",    label:"კატეგორია",fn:r => { const c = cats.find(x => x.id === r.catId); return <span style={{ fontSize:13 }}>{c?.icon} {c?.name}</span>; }},
          { k:"addr",   label:"ობიექტი",  fn:r => <APill addrId={r.addrId} dark={dark} /> },
          { k:"amount", label:"თანხა",    fn:r => <span style={{ fontWeight:700, color:G.accent, fontSize:14 }}>{cur}{r.amount.toFixed(2)}</span> },
          { k:"st",     label:"სტატუსი",  fn:r => <Badge status={r.status} dark={dark} /> },
        ...
        rows={fSpend}
        footer={[<span key="t" style={{ fontSize:13, color:G.textSecondary }}>ჯამი: <strong style={{ color:G.accent }}>{cur}{tot.toFixed(2)}</strong></span>]}
      />
    </>;
  }
  ...
  function PageReports() {
    ...
    return <>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:12, marginBottom:20 }}>
        <KCard G={G} label="ინვენტარის ღირებულება" value={`${cur}${invVal.toFixed(0)}`} icon="💰" />
        <KCard G={G} label="სულ შესყიდვები" value={`${cur}${purch.reduce((s,p)=>s+p.qty*p.up,0).toFixed(0)}`} icon="🛒" />
        <KCard G={G} label="სულ ხარჯები" value={`${cur}${spends.reduce((s,sp)=>s+sp.amount,0).toFixed(0)}`} icon="💸" />
        <KCard G={G} label="კატეგორიები" value={cats.length} icon="🗂" />
      </div>
      <div style={{ background:G.card, borderRadius:12, padding:"20px 22px", boxShadow:G.shadow, marginBottom:18, border:`1px solid ${G.border}` }}>
        <h3 style={{ fontSize:15, fontWeight:700, color:G.text, margin:"0 0 18px" }}>📊 თვიური შესყიდვები / ხარჯები</h3>
        ...
        <div style={{ display:"flex", gap:18, marginTop:12 }}>
          <span style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:G.textSecondary }}><span style={{ width:12, height:12, background:G.blue, borderRadius:3, display:"inline-block" }}/> შესყიდვები</span>
          <span style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:G.textSecondary }}><span style={{ width:12, height:12, background:G.accent, borderRadius:3, display:"inline-block" }}/> ხარჯები</span>
        </div>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        <div style={{ background:G.card, borderRadius:12, padding:"18px 20px", boxShadow:G.shadow, border:`1px solid ${G.border}` }}>
          <h3 style={{ fontSize:15, fontWeight:700, color:G.text, margin:"0 0 14px" }}>📦 კატეგორიების ინვენტარი</h3>
          {cats.map(c => {
        ...
        <div style={{ background:G.card, borderRadius:12, padding:"18px 20px", boxShadow:G.shadow, border:`1px solid ${G.border}` }}>
          <h3 style={{ fontSize:15, fontWeight:700, color:G.text, margin:"0 0 14px" }}>🏗 ობიექტების ხარჯები</h3>
          {ADDRESSES.map(a => {
    ...
    return <>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:16 }}>
        <div style={{ background:G.card, borderRadius:12, padding:"20px", boxShadow:G.shadow, border:`1px solid ${G.border}` }}>
          ...
          <Toggle G={G} label="მცირე მარაგის შეტყობინება" desc="გაფრთხილება მინიმალურ მარაგზე" value={cfg.lowAlert} onChange={() => setCfg(p => ({...p, lowAlert:!p.lowAlert}))} />
          <Toggle G={G} label="ელ-ფოსტის შეტყობინება" desc="შეტყობინება ელ-ფოსტაზე" value={cfg.emailNotif} onChange={() => setCfg(p => ({...p, emailNotif:!p.emailNotif}))} />
          <Toggle G={G} label="დამტკიცების სისტემა" desc="ხარჯების ადმინისტრაციული დამტკიცება" value={cfg.approval} onChange={() => setCfg(p => ({...p, approval:!p.approval}))} />
        </div>
        <div style={{ background:G.card, borderRadius:12, padding:"20px", boxShadow:G.shadow, border:`1px solid ${G.border}` }}>
          <h3 style={{ fontSize:15, fontWeight:700, color:G.text, margin:"0 0 14px" }}>💱 ფინანსური პარამეტრები</h3>
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <Inp G={G} label="ვალუტის სიმბოლო" value={cfg.cur} onChange={e => setCfg(p => ({...p, cur:e.target.value}))} />
            <Inp G={G} label="დღგ %" type="number" value={cfg.tax} onChange={e => setCfg(p => ({...p, tax:e.target.value}))} />
            <Sel G={G} label="ნაგულისხმევი ერთეული" value={cfg.defUnit} onChange={e => setCfg(p => ({...p, defUnit:e.target.value}))}>
        ...
        <div style={{ background:G.card, borderRadius:12, padding:"20px", boxShadow:G.shadow, border:`1px solid ${G.border}` }}>
          ...
          {[["პოზიციები",items.length],["შესყიდვები",purch.length],["ხარჯები",spends.length],["კატეგორიები",cats.length]].map(([l,v]) => (
          ...
          <Btn G={G} variant="secondary" style={{ width:"100%", marginTop:14 }}
            onClick={() => { if (window.confirm("მონაცემების განახლება?")) { setItems(ITEMS0); setPurch(PURCH0); setSpends(SPENDS0); setCats(INIT_CATS); }}}>
    ...
    <div style={{ display:"flex", minHeight:"100vh", fontFamily:FONT, background:G.bg, fontSize:14 }}>
      ...
      <div style={{ width:collapsed?62:218, background:G.sidebar, display:"flex", flexDirection:"column", transition:"width .2s", flexShrink:0, position:"sticky", top:0, height:"100vh", borderRight:`1px solid ${G.sidebarBorder}` }}>
        <div onClick={() => setPage("dashboard")}
          style={{ padding:collapsed?"14px 0":"18px 16px", display:"flex", alignItems:"center", gap:12, borderBottom:`1px solid ${G.sidebarBorder}`, justifyContent:collapsed?"center":"flex-start", minHeight:62, cursor:"pointer" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
        ...
        <div style={{ background:G.topbar, borderBottom:`1px solid ${G.border}`, padding:"0 24px", display:"flex", alignItems:"center", justifyContent:"space-between", height:56, position:"sticky", top:0, zIndex:10 }}>
          <div>
            ...
            <div style={{ fontSize:11, color:G.textMuted, fontWeight:600 }}>სტრუქტურა დეველოპმენტი · {now.toLocaleDateString("ka-GE")}</div>
          </div>
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            ...
            <div style={{ position:"relative" }}>
              ...
              {notifOpen && (
                <div style={{ position:"absolute", right:0, top:44, width:320, background:G.card, borderRadius:12, boxShadow:"0 8px 28px rgba(0,0,0,.2)", border:`1px solid ${G.border}`, zIndex:100 }}>
                  <div style={{ padding:"12px 16px", borderBottom:`1px solid ${G.border}`, fontWeight:700, fontSize:14, color:G.text }}>შეტყობინებები ({notifs.length})</div>
                  <div style={{ maxHeight:280, overflowY:"auto" }}>
                    {notifs.length === 0
                      ? <div style={{ padding:"20px", textAlign:"center", color:G.textMuted, fontSize:13 }}>✅ ყველა ნორმალურია</div>
      ...
      {/* MODALS */}
      {modal === "item" && (
        <Modal G={G} title="➕ ახალი ინვენტარის პოზიცია" onClose={() => setModal(null)}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:13 }}>
            <div style={{ gridColumn:"1/-1" }}><Inp G={G} label="სახელი *" value={iF.name} onChange={e => setIF(p => ({...p, name:e.target.value}))} placeholder="მაგ. ცემენტი M400" /></div>
            <Sel G={G} label="კატეგორია *" value={iF.catId} onChange={e => setIF(p => ({...p, catId:e.target.value}))}>{cats.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}</Sel>
            ...
            <Inp G={G} label="საწყისი რაოდენობა *" type="number" value={iF.qty} onChange={e => setIF(p => ({...p, qty:e.target.value}))} placeholder="0" />
            <Sel G={G} label="ერთეული" value={iF.unit} onChange={e => setIF(p => ({...p, unit:e.target.value}))}>{UNITS.map(u => <option key={u}>{u}</option>)}</Sel>
            <Inp G={G} label="მინიმალური მარაგი" type="number" value={iF.minQ} onChange={e => setIF(p => ({...p, minQ:e.target.value}))} placeholder="0" />
            <Inp G={G} label={`ფასი/ერთეული (${cur})`} type="number" value={iF.price} onChange={e => setIF(p => ({...p, price:e.target.value}))} placeholder="0.00" />
            <div style={{ gridColumn:"1/-1" }}><Txa G={G} label="შენიშვნა" value={iF.notes} onChange={e => setIF(p => ({...p, notes:e.target.value}))} placeholder="..." /></div>
          </div>
          {iF.qty && iF.price && <div style={{ background:dark?"#0A1F10":"#F0FDF4", border:`1px solid ${dark?"#14532D":"#BBF7D0"}`, borderRadius:8, padding:"10px 14px", marginTop:12, fontSize:14, fontWeight:700, color:G.green }}>💰 ჯამური ღირებულება: {cur}{(+iF.qty * +iF.price).toFixed(2)}</div>}
      ...
      {modal === "purch" && (
        <Modal G={G} title="🛒 ახალი შესყიდვა" onClose={() => setModal(null)}>
          <div style={{ background:dark?"#080F24":"#EFF6FF", border:`1px solid ${dark?"#1e3a8a":"#BFDBFE"}`, borderRadius:8, padding:"10px 14px", marginBottom:14, fontSize:13, color:dark?"#93C5FD":"#1E40AF", fontWeight:600 }}>⚠️ შეარჩიეთ ობიექტი — ავტომატური ინვენტარის განახლება</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:13 }}>
            <Inp G={G} label="თარიღი *" type="date" value={pF.date} onChange={e => setPF(p => ({...p, date:e.target.value}))} />
            <Sel G={G} label="ობიექტი *" value={pF.addrId} onChange={e => setPF(p => ({...p, addrId:e.target.value}))}>{ADDRESSES.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</Sel>
            <div style={{ gridColumn:"1/-1" }}><Inp G={G} label="საქონლის სახელი *" value={pF.item} onChange={e => setPF(p => ({...p, item:e.target.value}))} placeholder="მაგ. ცემენტი M400" /></div>
            <Sel G={G} label="კატეგორია" value={pF.catId} onChange={e => setPF(p => ({...p, catId:e.target.value}))}>{cats.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}</Sel>
            <Inp G={G} label="მომწოდებელი" value={pF.sup} onChange={e => setPF(p => ({...p, sup:e.target.value}))} placeholder="კომპანიის სახელი" />
            <Inp G={G} label="რაოდენობა *" type="number" value={pF.qty} onChange={e => setPF(p => ({...p, qty:e.target.value}))} placeholder="0" />
            <Sel G={G} label="ერთეული" value={pF.unit} onChange={e => setPF(p => ({...p, unit:e.target.value}))}>{UNITS.map(u => <option key={u}>{u}</option>)}</Sel>
            <Inp G={G} label={`ერთეულის ფასი (${cur}) *`} type="number" value={pF.up} onChange={e => setPF(p => ({...p, up:e.target.value}))} placeholder="0.00" />
            <Sel G={G} label="სტატუსი" value={pF.status} onChange={e => setPF(p => ({...p, status:e.target.value}))}>{Object.keys(STATUS_META).map(s => <option key={s}>{s}</option>)}</Sel>
            <div style={{ gridColumn:"1/-1" }}><Txa G={G} label="შენიშვნა" value={pF.notes} onChange={e => setPF(p => ({...p, notes:e.target.value}))} placeholder="..." /></div>
          </div>
          {pF.qty && pF.up && <div style={{ background:dark?"#0A1F10":"#F0FDF4", border:`1px solid ${dark?"#14532D":"#BBF7D0"}`, borderRadius:8, padding:"10px 14px", marginTop:12, fontSize:14, fontWeight:800, color:G.green }}>✅ ჯამი: {cur}{(+pF.qty * +pF.up).toFixed(2)} | დღგ: {cur}{(+pF.qty * +pF.up * +cfg.tax / 100).toFixed(2)}</div>}
          <div style={{ display:"flex", gap:10, marginTop:16, justifyContent:"flex-end" }}>
            <Btn G={G} variant="secondary" onClick={() => setModal(null)}>გაუქმება</Btn>
      ...
      {modal === "spend" && (
        <Modal G={G} title="💸 ახალი ხარჯი" onClose={() => setModal(null)}>
          <div style={{ background:dark?"#1F0808":"#FFF5F5", border:`1px solid ${dark?"#7F1D1D":"#FED7D7"}`, borderRadius:8, padding:"10px 14px", marginBottom:14, fontSize:13, color:dark?"#FCA5A5":"#991B1B", fontWeight:600 }}>💸 ობიექტის ხარჯი{cfg.approval ? " — ადმინისტრაციული დამტკიცება საჭიროა" : ""}</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:13 }}>
            <Inp G={G} label="თარიღი *" type="date" value={sF.date} onChange={e => setSF(p => ({...p, date:e.target.value}))} />
            <Sel G={G} label="ობიექტი *" value={sF.addrId} onChange={e => setSF(p => ({...p, addrId:e.target.value}))}>{ADDRESSES.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</Sel>
            <div style={{ gridColumn:"1/-1" }}><Inp G={G} label="აღწერა *" value={sF.desc} onChange={e => setSF(p => ({...p, desc:e.target.value}))} placeholder="მაგ. ბეტონი II სართული" /></div>
            <Sel G={G} label="კატეგორია" value={sF.catId} onChange={e => setSF(p => ({...p, catId:e.target.value}))}>{cats.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}</Sel>
            <Inp G={G} label={`თანხა (${cur}) *`} type="number" value={sF.amount} onChange={e => setSF(p => ({...p, amount:e.target.value}))} placeholder="0.00" />
            <Sel G={G} label="სტატუსი" value={sF.status} onChange={e => setSF(p => ({...p, status:e.target.value}))}>{Object.keys(STATUS_META).map(s => <option key={s}>{s}</option>)}</Sel>
            <div style={{ gridColumn:"1/-1" }}><Txa G={G} label="შენიშვნა" value={sF.notes} onChange={e => setSF(p => ({...p, notes:e.target.value}))} placeholder="..." /></div>
          </div>
          <div style={{ display:"flex", gap:10, marginTop:16, justifyContent:"flex-end" }}>
            <Btn G={G} variant="secondary" onClick={() => setModal(null)}>გაუქმება</Btn>
      ...
      {modal === "cat" && (
        <Modal G={G} title="🗂 ახალი კატეგორია" onClose={() => setModal(null)} width={360}>
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <Inp G={G} label="სახელი *" value={cF.name} onChange={e => setCF(p => ({...p, name:e.target.value}))} placeholder="კატეგორიის სახელი" />
        ...
        return (
          <Modal G={G} title={`📦 ${detItem.name}`} onClose={() => { setModal(null); setDetItem(null); }} width={400}>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
              {[["კატეგორია",`${c?.icon} ${c?.name}`],["ობიექტი",a?.name.split(" – ")[0]],["რაოდენობა",`${detItem.qty} ${detItem.unit}`],["მინ. მარაგი",`${detItem.minQ} ${detItem.unit}`],["ფასი/ერთ.",`${cur}${detItem.price}`],["ჯამური ღირებ.",`${cur}${(detItem.price*detItem.qty).toFixed(2)}`]].map(([l,v]) => (
            ...
            <div style={{ marginBottom:14 }}>
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:G.textMuted, marginBottom:4 }}>
                <span>მარაგის დონე</span>
                <span style={{ fontWeight:700, color:low?G.accent:G.green }}>{low ? "⚠ მცირეა" : "✅ ნორმალურია"}</span>
              </div>
            ...
            <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}>
              <Btn G={G} size="sm" variant="secondary" onClick={() => { setModal(null); setDetItem(null); }}>დახურვა</Btn>
              <Btn G={G} size="sm" onClick={() => { setModal("purch"); setDetItem(null); }}>🛒 შესყიდვა</Btn>
            </div>
          </Modal>
        );
      })()}
